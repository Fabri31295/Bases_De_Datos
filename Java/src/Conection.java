...
public class Conection {
	...
	public boolean connectToDatabase(DBTable table, String username, String password) {
		try {
			...
			if (username.equals("admin")) {
				...
				if (!connectAsInspector(table, driver, url, username, password)) {
					return false;
				}
			}
			return true;
	...
	private boolean connectAsInspector(DBTable table, String driver, String url, String username, String password) {
		try {
			table.connectDatabase(driver, url, "inspector", "inspector");
			connection = DriverManager.getConnection(url, "inspector", "inspector");
			String query = "SELECT legajo, password FROM inspector WHERE  password = md5('" + password + "') AND "
					+ username + " = legajo;";
			...
			String msj = "Error al intentar conectarse a la base de datos.\nPor favor, revise el usuario y contraseña";
	...
	public void disconnectDataBase(DBTable table) {
		try {
			...
			printSqlException(ex, "Error de desconexion.", "Error");
		}
	}
	...
	public void refreshExcecute(String message, JComboBox<String> comboBox, Main panel) {
		try {
			...
			while (rs.next()) {
				...
				if (message.contains("Fecha")) {
					str = Date.convertirDateAString(rs.getDate(1));
				} else {
					str = rs.getString(1);
				}
	...
	private void printSqlException(SQLException ex, String msj, String description) {
		...
		JOptionPane.showMessageDialog(Main.getInstance(), msj, description, JOptionPane.ERROR_MESSAGE);
	}
